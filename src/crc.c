...
void append_crc_16(char *buffer,int *len)
{
  int crc;
  int i;
  char *bufptr;
  ...
  crc = 0;
  for (i=0;i<*len;i++) {
    crc = (crc >> 8) ^ Crc_16_table[(crc ^ *bufptr++) & 0xff];
  }
  *bufptr++ = crc;
  *bufptr++ = (crc >> 8);
  *len += 2;
...
int check_crc_16(char *buffer,int *len)
{
  int crc;
  int i;
  char *bufptr;
...
void append_crc_rmnc(char *buffer,int *len)
{
  int crc;
  int i;
  char *bufptr;
  ...
  crc = 0xFFFF;
  for (i=0;i<*len;i++) {
    crc = (crc << 8) ^ Crc_rmnc_table[((crc >> 8) ^ *bufptr++) & 0xff];
  }
  *bufptr++ = (crc >> 8);
  *bufptr++ = crc;
  *len += 2;
...
int check_crc_rmnc(char *buffer,int *len)
{
  int crc;
  int i;
  char *bufptr;
  ...
  if ((crc & 0xFFFF) != 0x7070) return(1);
  *len -= 2;
  return(0);
}

/* CRC for AXIP, taken from crc.c (axip-package) */
